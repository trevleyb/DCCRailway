...
public class CommandStationFactory(ILogger logger) {
    ...
    private void LoadControllers() {
        ...
        if (!Directory.Exists(path))
            throw new ApplicationException(
                "[Controllers] Invalid Path provided for the CommandStation Assembly Search");
        var assemblies = Directory.GetFiles(DefaultPath).Where(directory => Regex.Match(directory, pattern).Success)
            .ToList();
        if (assemblies == null || assemblies.Any() == false)
            throw new ApplicationException(
                $"[Controllers] Could not find any CommandStation Assemblies '{path} => {pattern}'");
        ...
        foreach (var assemblyPath in assemblies)
            try {
                ...
                foreach (var controller in assembly.DefinedTypes.Where(
                             t => t.ImplementedInterfaces.Contains(typeof(ICommandStation))))
                    ...
                    try {
                        var systemAttr =
                            AttributeExtractor
                                .GetAttribute<
                                    ControllerAttribute>(
                                    controller); //(SystemNameAttribute) Attribute.GetCustomAttribute(dccSystem, typeof(SystemNameAttribute))!;
                        if (systemAttr is not null) {
                            if (_controllers.ContainsKey(systemAttr.Name))
                                throw new ApplicationException(
                                    $"Duplicate CommandStation Name found: {systemAttr.Name}");
                            _controllers.Add(systemAttr.Name,
                                             new CommandStationManager(logger, systemAttr, assemblyPath, controller));
                        }
                    }
                    catch (Exception ex) {
                        logger.Debug(
                            "ASSEMBLY: Unable to obtain the name of the Manufacturer or CommandStation from the Assembly.",
                            ex);
                    }
            }
            catch (Exception ex) {
    ...
    public List<CommandStationManager> FindByManufacturer(string manufacturer) {
        return _controllers
            ?.Where(key => key.Value.Manufacturer.Equals(manufacturer, StringComparison.InvariantCultureIgnoreCase))
            .Select(key => key.Value).ToList() ?? [];
    }

    public CommandStationManager? Find(string name) {
        return Controllers?.FirstOrDefault(key => key.Name.Equals(name, StringComparison.InvariantCultureIgnoreCase)) ??
               null;
    }
}