...
public class CommandStationFactory {
    ...
    private void LoadControllers() {
        ...
        // ---------------------------------------------------------------------------------------------------------
        if (!Directory.Exists(path)) throw new ApplicationException("[Controllers] Invalid Path provided for the CommandStation Assembly Search");
        var assemblies = Directory.GetFiles(DefaultPath).Where(directory => Regex.Match(directory, pattern).Success).ToList();
        if (assemblies == null || assemblies.Any() == false) throw new ApplicationException($"[Controllers] Could not find any CommandStation Assemblies '{path} => {pattern}'");
