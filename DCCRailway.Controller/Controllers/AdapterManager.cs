...
public class AdapterManager(ILogger logger, ICommandStation commandStation, Assembly assembly) {
    ...
    public IAdapter? Attach(string? adapterName) {
        ...
        try {
            foreach (var adapters in _adapters)
                if (adapters.Value.Name != null &&
                    adapters.Value.Name.Equals(adapterName, StringComparison.InvariantCultureIgnoreCase)) {
    ...
    public bool IsAdapterSupported(string name) {
        return _adapters.Any(pair => pair.Value.Name != null &&
    ...
    private void RegisterAdapters() {
        ...
        foreach (var adapter in foundTypes) {
            ...
            if (attr != null && !string.IsNullOrEmpty(attr.Name)) {
                var adapterInterface = adapter.ImplementedInterfaces.First(
                    x => x.FullName != null &&
                         x.FullName.StartsWith("DCCRailway.Controller.Adapters.",
    ...
    private void OnAdapterRemoved(object sender, IAdapter adapter) {
        var e = new AdapterEventArgs(adapter, AdapterEventType.Detatch, null,
                                     $"Adapter {adapter.GetType().Name} removed");
        AdapterEvent.Invoke(sender, e);
    }

    private void OnAdapterDataRecv(object sender, IAdapter adapter, DataRecvArgs dataRecvArgs) {
        var e = new AdapterEventArgs(adapter, AdapterEventType.DataRecv, dataRecvArgs.Data,
                                     $"Adapter {adapter.GetType().Name} recieved data.");
        AdapterEvent.Invoke(sender, e);
    }

    private void OnAdapterDataSent(object sender, IAdapter adapter, DataSentArgs dataSentArgs) {
        var e = new AdapterEventArgs(adapter, AdapterEventType.DataRecv, dataSentArgs.Data,
                                     $"Adapter {adapter.GetType().Name} sent data");
        AdapterEvent.Invoke(sender, e);
    }

    private void OnAdapterError(object sender, IAdapter adapter, DataErrorArgs dataErrorArgs) {
        var e = new AdapterEventArgs(adapter, AdapterEventType.DataRecv, null,
                                     $"Adapter {adapter.GetType().Name} recieved error: " + dataErrorArgs.Error);
        AdapterEvent.Invoke(sender, e);
    }
