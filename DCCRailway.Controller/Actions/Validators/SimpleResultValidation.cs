...
public class SimpleResultValidation : IResultValidation {
    ...
    public ICmdResult Validate(byte[]? dataReceived) {
        ...
        if (dataReceived?.Length > LengthExpected) {
            return CmdResult.Fail(dataReceived!, $"Too much data was recieved. Expected {LengthExpected} but recieved {dataReceived?.Length}");
        }

        if (dataReceived?.Length < LengthExpected) {
            return CmdResult.Fail(dataReceived!, $"Not enough data was recieved. Expected {LengthExpected} but only recieved {dataReceived?.Length}");
        }
