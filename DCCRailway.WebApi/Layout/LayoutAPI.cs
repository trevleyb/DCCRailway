...
public static class APIHelper {
    public static void MapEntity<TCollection, TEntity>(WebApplication app, string endPointName)
        where TEntity : ILayoutEntity
        where TCollection : ILayoutRepository<TEntity> {
        // Register services
        ...
        app.MapGet(GetEndPointName(endPointName, "id"), (TCollection entities, string id) => {
            ...
            if (entity is null) {
                return Results.NotFound($"{endPointName} with ID {id} not found");
            }
        ...
        app.MapPut(GetEndPointName(endPointName, "id"),
                   async (TCollection entities, string id, TEntity updatedEntity) => {
