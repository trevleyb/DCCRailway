...
public class Server(ILogger logger, IRailwaySettings railwaySettings, StateManager.StateManager stateManager) {
    ...
    /// </summary>
    public IResult Start(ICommandStation commandStation) {
        ...
        var result = CheckForRunningService(_hostPort);
        ...
        return Result.Ok("Service is now running in the background. ");
    }
    ...
    private void SetupHeartbeatTimer() {
        if (railwaySettings.Settings.WiThrottle.HeartbeatCheckTime > 0) {
    ...
    private void SetupFastClockTimer() {
        if (railwaySettings.WiThrottlePrefs.UseFastClock) {
            _fastClockTimer           =  new Timer(railwaySettings.Settings.WiThrottle.FastClockCheckTime * 1000);
    ...
    private IResult CheckForRunningService(int port) {
        if (!ServiceHelper.ServiceHelper.IsServiceRunningOnPort(_hostPort)) return Result.Ok();

        // Make sure that the Service is not already running
        // -------------------------------------------------------
        var retries = 0;
        while (retries++ < 5 && ServiceHelper.ServiceHelper.IsServiceRunningOnPort(_hostPort)) {
            logger.ForContext<Server>().Error($"Service is already running. Attemp {retries} to shut it down.");
            var result = ServiceKill.KillService(_hostPort);
        }

        return ServiceHelper.ServiceHelper.IsServiceRunningOnPort(_hostPort) ? Result.Fail("Unable to start the service as it is already running.") : Result.Ok();
    }
}