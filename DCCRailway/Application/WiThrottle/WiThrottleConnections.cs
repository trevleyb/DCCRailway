...
public class WiThrottleConnections : List<WiThrottleConnection> {
    ...
    /// </summary>
    public WiThrottleConnection Add(TcpClient client, WiThrottlePreferences preferences, IRailwayConfig railwayConfig, IController commandStation) {
        var connection = Find((ulong)client.Client.Handle);
        if (connection is null) {
            connection = new WiThrottleConnection(client, preferences, this, railwayConfig, commandStation);
            Add(connection);
        }
        return connection;
    }
