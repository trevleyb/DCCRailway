...
public class SimpleResultValidation : IResultValidation {
    ...
    public IResult Validate(byte[] dataRecieved) {
        ...
        if (dataRecieved?.Length > LengthExpected) return new ResultError($"Too much data was recieved. Expected {LengthExpected} but recieved {dataRecieved?.Length}", dataRecieved!);
        if (dataRecieved?.Length < LengthExpected) return new ResultError($"Not enough data was recieved. Expected {LengthExpected} but only recieved {dataRecieved?.Length}", dataRecieved!);
