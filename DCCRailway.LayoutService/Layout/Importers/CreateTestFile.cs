...
namespace DCCRailway.LayoutService.Layout.Importers;

public static class CreateTestFile {

        public static void Build(string filename) {

        var manager = new LayoutRepositoryManager();
        ...
        var route = new Layout.Entities.Route { Id = "R01", Name = "Route1", Description = "Station to Mainline" };
        route.AddRoute("T01",true);
        route.AddRoute("T02",true);
        routes.Add(route);

        route = new Layout.Entities.Route { Id = "R02", Name = "Route2", Description = "Yard to Mainline" };
        route.AddRoute("T01",false);
        route.AddRoute("T02",true);
        routes.Add(route);

        route = new Layout.Entities.Route { Id = "R03", Name = "Route3", Description = "Yard to Station" };
        route.AddRoute("T01",true);
        route.AddRoute("T02",false);
        routes.Add(route);

        route = new Layout.Entities.Route { Id = "R04", Name = "Route4", Description = "Mainline to Station" };
        route.AddRoute("T01",false);
        route.AddRoute("T02",false);
        routes.Add(route);
