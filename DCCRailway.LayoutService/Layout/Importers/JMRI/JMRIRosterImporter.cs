...
namespace DCCRailway.LayoutService.Layout.Importers.JMRI;

public static class JMRIRosterImporter {
    public static void Import(LayoutRepositoryManager repositoryManager, string rosterName) {
        ...
        try {
            ...
            MapJMRItoDCCTrain(repositoryManager, jmriRoster);
        }
        catch (Exception ex) {
    ...
    private static async void MapJMRItoDCCTrain(LayoutRepositoryManager repositoryManager, JMRIRoster jmriRoster) {

        var locomotives = repositoryManager.Locomotives;
        locomotives.Clear();
